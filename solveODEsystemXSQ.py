...
# -*- coding: utf-8 -*-
"""
Created on Mon May  6 18:03:13 2019
...
class solveODEsystem:
    ...
    """
    def __init__(self, kappa=0.24, alpha=0.16, phi=0.7, A=10, Q_sigma=0.2,
                 N_T=10, n_samples=1000): 
    ...
    def solveH1(self):
        ...
        def u():
            ...
            for i in range(1, self.N_T+1):
                u_t[i-1] = np.exp(np.sum(np.divide(np.add(self.h2[:i], self.alpha), 2.*self.kappa)*self.dt))
            return u_t
